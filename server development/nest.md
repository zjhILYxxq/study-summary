...
3. 前台执行模式和守护进程执行模式？ 
    ...
    前台执行是可见的，会占用当前 `termal` 窗口，使窗口不能再执行别的命令和任务,可以用 `ctrl + c` 中断, 关闭 `termal` 窗口会导致任务终止。
    
    守护进程就是可以在后台运行，由守护进程保证任务的不间断，当系统检测到守护进程中进程终止之后会自动再拉起这个进程或者任务。

4. 什么是负载均衡？ 什么是惊群现象？

5. 使用 pm2 管理服务时，涉及到哪些进程？

    涉及的进程分为三类: 
    - 执行 `pm2` 命令的进程 - `Satan` 进程， 撒旦？
    - 守护进程 - `God`
    - 服务进程

    其中， `Satan` 进程通过 `rpc` 和 `God` 进程进行通信， `God` 进程和服务进程又怎么通信呢？
