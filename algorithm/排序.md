...
- **堆排序**
    ...
    **堆排序**的思路：
    1. 把**无序数组**构建成为**二叉堆**(**从大到小**排序，需要构建为**最大堆**；**从小到大**排序，需要构建为**最小堆**)；
    ...
    // 堆顶元素下沉
    function siftDown(heap, parentIndex, length) {
        let temp = heap[parentIndex];
        let childIndex = 2 * parentIndex + 1;
        while (childIndex < length) {
            if (childIndex + 1 < length && heap[childIndex] < head[childIndex + 1]) {
                childIndex++;
            }
