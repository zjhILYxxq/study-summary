...
- **单序列问题**，题目的输入通常是一个一维数组或者字符串
  ...
  - [x] [最长斐波那契数列](https://leetcode.cn/problems/Q91FMA/) - 解题思路，dp[i][j] = dp[j][k] + 1, dp[i][j] 代表最后一位为 i， 前一位为 j 的斐波那契数列， d[j][k] 为最后一位为 j，前一位为 k 的斐波那契数列。解题时，需要建立一个 n * n 的 dp 数组。(这个题，最后回来看一下)
...
- **双序列问题**
  ...
  - [ ] [子序列的数目](https://leetcode.cn/problems/21dk04/) - 困难
