...
诚然这是一个有效的办法，但在实际使用时却存在非常大的局限性。也许，我们自己访问站点的时候性能很好，没有什么问题，但用户在实际访问时，由于设备、网络、使用姿势、使用人数、使用时间段、服务吞吐量等原因，整个体验很可能会没有达到我们的预期。这种情况下进行性能分析非常麻烦，首先我们无法感知，其次我们也无法在本地直接复现出用户的使用情形。那怎么办呢，😂?

这个时候，我们可以借助性能监控工具来处理这个问题，如 `Sentry`、`Fundebug` 等(当然，也可以自研)。这类工具，可以在用户访问站点时，将首屏性能、用户交互涉及的指标通过接口上报给后台管理系统。后台系统接受到上报数据以后，对数据做汇总、计算，然后以可视化图表的方式展示。通过这些图表，我们就可以进行性能分析，找到影响用户体验的因素，非常方便。

本文，小编将以 `Sentry` 为例，和大家一起聊聊 `Sentry` 是如何做性能监控的, 希望能给到大家一些启发。

本文的目录结构如下:

- **<a href="#1">常见的性能监控指标</a>**

- **<a href="#2">Sentry 如何配置前端性能监控的</a>**

- **<a href="#3">Sentry 性能监控的原理</a>**
...
不过，光有这些还不够。上面的这些指标，更多是面向开发人员来说的，如果从用户角度来看，不够形象，并且难以理解。实际上，从用户角度来说，用户更加关心页面何时开始渲染、何时渲染出主要内容、何时可以交互、交互的时候是否有延迟、页面是否有抖动(闪烁) ？

针对用户关心的这几个指标，官方也提供了对应的指标和相应的获取方式:

- 页面何时开始渲染

    衡量页面何时开始渲染，有两个指标: `FP` 和 `FCP`:

    - `FP`, `first paint`, 表示页面开始首次绘制的时间点。在 `FP` 时间点之前，用户看到的是导航之前的页面。

    - `FCP`, `first contentful paint`, `lighthouse` 面板的六大指标之一，表示首次绘制任何文本、图像、非空白 canvas 或者 SVG 的时间点。官网资料: [FCP](https://web.dev/fcp/)。
...
- 页面何时渲染主要内容
    ...
    衡量页面何时渲染主要内容，有三个指标: `FMP`、`LCP` 和 `SI`:

    - `FMP`，`first meaningful paint`, 首次完成有意义内容绘制的时间点，官方资料: [FMP](https://web.dev/first-meaningful-paint/)。
  
    - `SI`, `speed index`, 速度指标, `lighthouse` 面板中的六大指标之一，用于衡量页面加载期间内容的绘制速度， 官方资料: [SI](https://web.dev/speed-index/)。
  
    - `LCP`， `lagest contentful paint`， `lighthouse` 面板中的六大指标之一，完成最大内容绘制的时间点，官方资料: [LCP](https://web.dev/lcp/)。
...
- 何时可以交互
    ...
    `TTI`, `time to ineractive`, 可交互时间， `lighthouse` 面板中的六大指标之一, 用于测量页面从开始加载到主要子资源完成渲染，并能够快速、可靠地响应用户输入所需的时间。 官方资料: [TTI](https://web.dev/i18n/zh/tti/)。

    和 `FMP`、`SI` 一样，官方并没有提供获取 `TTI` 的有效接口。不过没有关系，我们可以根据官方提供的计算过程自己计算。
    ...
    在这个计算过程中，最关键的两点: 收集 `long task` 和找到安静窗口。
    ...
    new PerformanceObserver((entryList) => {
        for (const entry of entryList.getEntries()) {
            ...
            longTask.push(entry.startTime + entry.duration);
        }
    }).observe({type: 'longtask', buffered: true});
    ```
    安静窗口的实现就有点复杂了。其中，不超过 2 个网络请求的限制，需要我们通过拦截网络请求加一个请求 poll 实现。

    ```
    // 我们需要拦截 xhr、fetch、静态资源这三类请求
    ...
    通常，如果一个任务在主线程上运行的时间超过 `50` ms，主线程就被视作阻塞状态, 对应的该任务的`阻塞时间 = 任务运行时间 - 50`。`TBT` 是从 `FCP` 到 `TTI` 之间所有任务的阻塞时间之和。
...
<h3 id="2">Sentry 如何配置前端性能监控的</h3>



<h3 id="3">Sentry 性能监控原理</h3>
...
- [SI](https://web.dev/speed-index/) 

- [LCP](https://web.dev/lcp/)

- [TTI](https://web.dev/i18n/zh/tti/)
