...
- 项目介绍
  ...
  - [x] `SaaS` 项目准备
    ...
    2. 为什么之前 `SaaS` 之前用的是 `ssr` 架构？ 
       ...
       - 通过一个简单的 BFF 层，可以实现如下功能:
        
       - 统一获取用户的权限信息、灰度信息、用户基本信息，在 bff 层获取要比浏览器获取更快；
  
            这些数据怎么使用？

            通过中间件获取，添加到 req 中, 然后在页面组件的 getInitialProps 中从 req 中获取，通过 props 传递给组件。

            传递给组件时，这部分数据实际是存到 html 页面中了，等到 client 进行 hydreate 的时候，会从 html 中获取这部分数据。

         - 做一些接口聚合、接口缓存；

         - 利用中间件，做一些复杂业务的逻辑判断，如重定向、免登；
    ...
    6. 如何解决 qiankun 开启沙箱后子应用重新激活出现页面抖动问题。
    ...
    10. 有处理过什么样子的 `Sentry` 上报异常

      通过 `Sentry` 还捕获到我们系统中的一些边界异常

      示例1: 以一通呼入通话为例，这通通话有可能在客户还没有接听的时候用户就挂断了，然后客户此时也选择挂断的话，会提示一个 `session.reject` 方法不存在问题。

      由于用户点击挂断的时候，呼入弹窗会自动关闭，通话也会结束，所以这个问题并不是很容易被感知。

      解决方法也很简单: 调用 `session.reject` 方法之前，判断一下 `session.reject` 是否存在。究其原因，ts 类型检查不完善, 也算是历史遗留问题了。


      示例2: 测试环境 `bff` 层接口一直报错，导致返回服务端一直返回 `502`。原因是运维迁移了域名迁移的相关工作，而没有告知后端开发。

      示例3: 无法读取某个属性问题，查看发现是接口没有返回这个数据，然后访问这个属性的某个具体属性就会报错。后端接口返回数据有问题。解决方案: 接口定义的时候要明确 `ts` 类型。

      示例4： 接口参数问题，有些操作需要打点，会发现上报接口时传递的 `instanceId` 为 `0`，然后会抛出接口参数不正确异常。原因，某些上报的操作是，有其他操作把这个 `instance` 给重置了。解决方法：在代码逻辑里面，缓存 `instanceId`。
    ...
    11. SaaS 业务相关
      ...
      - 创建外呼任务: 选择话术、线路、要使用多少个话术机器人、是否开启高并发；
      - 发起外呼任务，
      - 语音对话交互(ASR、NLP、TTS、坐席监听、通话监听、AI 转人工)；
      - 数据分析统计

      话术机器人相关业务:
      - 配置一个话术机器人的时候，会有不同的节点，普通节点、跳转节点、按键采集节点、IVR 节点、条件判断节点等
      - 逻辑节点，会有不同的处理分支，需要根据用户的话语进判断，这个时候就会用到 NLP、ASR 技术；

      机器人是如何能识别用户的回答的含义的？

      在一些关键节点，我们会设置一些肯定、否定、默认的处理的分支。如果是肯定，会设置关键词，说明哪些词句代表肯定的意思；如果是否定，也会设置关键词，说明哪些语句代表否定的意思。如果是在是是被不出来，会走一些默认的处理分支。

      一个话术机器人创建好一些，会不停的训练。训练有两种方式，语音训练，文字训练。在这个过程中，会持续添加关键字。等训练好以后，才能把这个话术发布。然后才能使用这个话术和客户进行通话。

      通常，我们的话术会有专门的话术人员给客户配置。

      线路配置业务:
      - 在使用我们的 sass 系统开展业务的时候，需要给客户配置线路；
      - 线路分类实体线路和虚拟线路，实体线路是专门的线路配置的真实可用线路，虚拟线路是由多条实体线路组成，是给客户使用的线路；
      - 真实线路最底层是三大运营商：移动、电信、联通；
      - 一条虚拟线路会配置多条实体线路，当某一条实体线路不可使用时，可以切换到另外的线路；
      - 配置线路，需要配置地区、城市、行业、频次、并发、可使用的时间段、计费单价、计费类型、线路用途，还有通信相关的一些配置如 `fs`、网关、`opensips` 等；

      高并发场景:
      - 在开启任务的时候，会为这个任务配置使用 `ai` 机器人的数量
      - 后端会根据客户选择的 `ai` 机器人数，开启并发外呼；

      通常，我们的系统会同时处理多个外呼任务 ，每个外呼任务都会配置使用 `ai` 机器人的数量。引擎端会通过自动扩容的方式， 根据当前需要的 `AI` 坐席量，去扩充引擎节点，最高可支持几十万的并发。整个系统用的是阿里云的 `serverless` 服务和 `K8S` 服务。

      具体实现方式: 通过分流，拆分并发到整体集群上，整体集群可以支撑几十万AI坐席同时并发。集群节点可以实现动态库容，预测可支持并发量不足时，实现动态扩容。
    ...
    12.  优化相关
      ...
      聊到前端性能优化，主要是两个方面：首屏性能 - 什么时候能看到页面内容、用户什么时候可以可以操作页面，交互性能 - 用户交互的流畅程度。

      首屏性能:
      - 资源获取优化: 缓存策略、cdn、压缩文件的体积、懒加载、合理的分包策略；
      - 不要阻塞浏览器渲染：异步 js；
      - 合理的浏览器渲染策略: ssr、ssg、csr；
      - 尽快开始渲染分支：不要有太多的同步接口；

      交互性能:
      - js 代码不要执行太长时间，web worker、分段执行；
      - 使用 react 框架时候防止不必要的重复渲染；
      - 不要一次性渲染太多的 dom 节点，增强渲染、虚拟列表；
      - 尽量避免重排重绘: 给元素设置 width、height，批量操作 dom 节点，频繁操作的 dom 节点脱离文档流、不要使用 js 操作动画 等；
      - 接口响应速度；
    ...
    16. 前端规范
      ...
      代码规范:
        - `eslint` + `ts` 检查；
        - 常量命名大写，以下划线连接；
        - 关键业务要填写注释、新创建的文件在头部要填写注释；
        - `utils` 里面的工具方法要为纯函数；
        - `ts` 要完整，`any` 要控制，要善于使用 `ts` 提供的高级类型，而不是简单的重复定义 `interface`， 枚举要尽量使用英文；
        - `hooks` 的依赖要做到依赖诚实；
        - 重复的逻辑代码，要第一时间抽离；
        - 不要有过大的组件、过大的函数，函数的参数不要过长；
        - `if` 逻辑复杂时，要及时重构历史债务；
      ...
      代码提交规范:
        - 提交代码之前，自己要先检查一遍；
        - 往 `master` 上合代码时，要通过 mr 的方式，不能个人直接合；
        - 合并 `mr` 的时候，代码审核人要审核，提意见；
        - 提交的 `commit` 信息要符合规范；

      项目文档情况:
        - 项目配置放在根目录下，业务代码放在 `src` 目录下；
  ...
  - [x] 当前架构有哪些是可以做升级的
    ...
    1. 子应用交互，可以有 `mf` -> `qiankun` 的手动加载子应用；
    2. 异常告警，不依赖于飞书告警，而是对接一个 `node` 应用，更加灵活；
    3. 微前端架构，可以更简单一些，主应用不用采用 `ssr`, 减低应用的复杂度；
    4. 可以接入 `service workers`，做一些性能优化；
    5. 目前的 `bff` 层并没有承接更多的业务，以后可以考虑利用 `node` 层承接一些内部业务；
