...
- [x] `weakMap` 和 `weakSet`
    ...
    `weakMap` 和 `map` 的区别:
    - `weakMap` 只接受对象作为键名，不接受其他类型作为键名，而 `map` 支持各种类型；
    - `weakMap` 键名指向的对象是弱引用，不计入垃圾回收，可以防止内存泄漏，因此 `weakMap` 的键名没有办法遍历、也无法 `clear`；

    同样的， `weakSet` 和 `set` 的区别:
    - `weakSet` 只能放置对象，不能是其他类型的值；
    - `weakSet` 的值是弱引用，不计入垃圾回收，可以防止内存泄漏，无法遍历、无法 `clear`；

    `weakMap`、`weakSet` 无法遍历、无法 `clear` 的原因: 弱引用，只要所引用的对象的其他引用被清除，垃圾回收机制就会释放该对象占据的内存，`weakMap` 里面对应的键名对象和对应的键值、`weakSet` 里面的值自动会消失，不用手动清理。因此无法遍历、无法 `clear`。

- [x] 浏览器并发请求限制

    对于同一个协议、域名、端口，浏览器允许同时打开多个 `TCP` 连接，一般上限为 `6` 个。而且，必须是前 `6` 个处理完之后，后续的 `6` 个才会处理。是这样的吗？

    同一个 `TCP` 连接允许发送多次 `http` 请求，但必须等待前一个请求的首个字节响应到达客户端。
...
- [x] `eval` 和 `new Function` 的区别
    ...
    `new Function` 中的代码执行时的作用域为全局作用域，不论它的在哪个地方调用的。

- [x] 大文件上传

    前端：二进制数据分片, `formData` ? `blob` 数据调用 `slice` 方法；
