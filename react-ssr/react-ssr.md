...
8. SSR 同构
    ...
    代码同构: 服务端和客户端供用同一份代码，但是打包方式不一样。 客户端采用类似单入口文件打包的方式打包， 服务端采用多入口文件的方式打包；

    数据同构: 前后端都能获取数据。给组件提供一个静态方法，服务端渲染时获取数据；脱水以后的组件返回给客户端时，同时将数据返回给客户端。客户端渲染时，如果组件定义了静态方法，就使用服务端直出的数据渲染；
