...
- [x] 使用 Vite 时如何解决开发环境和生产环境产物不一致问题？ 
    ...
    只所以产物不一样，主要是开发模式和生产模式对第三方依赖的处理方式不同，可能会出现问题。
    ...
    ESM CDN 方案不支持 tree shaking

- [x] 为什么 bundle-less 方案应用在生产环境中还不成熟

    - 浏览器兼容性 - 现有的方案不支持 ES3/ES5 降级
    - 性能问题 - 大量的文件请求，影响性能(尽管有 http2 多路复用); ESM CDN 方案不支持 tree shaking；
    - 模块化兼容方案
