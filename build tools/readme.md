...
- [x] 常见的打包工具对比
    ...
    - 缺点：目前生态还不如 webpack；也有一定的上手成本；本地开发模式启动以后，首屏、懒加载响应速度对比 webpack 会慢；
...
之所以要做`预构建`，是因为 `Vite` 是基于浏览器原生的 `ESM` 模块规范来实现 `dev server` 的，这就要求整个项目中涉及的所有源代码必须符合 `ESM` 规范才可以。

在实际开发过程中，业务代码我们可以严格按照 `ESM` 规范来编写，但第三方依赖就无法保证了，比如 `react`。这就需要我们通过 Vite 的预构建将非 ESM 规范的代码转换为符合 ESM 规范的代码。

另外，尽管有些第三方依赖已经符合 ESM 规范，但它是由多个子文件组成的，如 lodash-es。如果不做处理就直接使用，那么就会引发请求瀑布流，对页面性能来说，简直就是一场灾难。同样的，我们可以通过 Vite 的预构建，将第三方依赖内部的多个文件合并为一个，减少 http 请求数量，优化页面加载性能。

综上，Vite 的预构建，主要做了两件事情：

- 建将非 ESM 规范的代码转换为符合 ESM 规范的代码；

- 将第三方依赖内部的多个文件合并为一个，减少 http 请求数量；

关于预构建的过程，我们可以通过几个简单的 demo 来为大家演示一下：
