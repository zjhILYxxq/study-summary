...
- [ ] 二次预构建
   ...
   2. vite3.0 修复了首屏时的二次预构建。
      ...
      首先，不管是首屏还是懒加载，只要发现为预构建的第三方依赖，都会进行二次构建，只不过首屏的二次构建不会重新 load 页面，而懒加载会重新 load 页面。

      个人猜测: 首屏是先把需要的内容，返回给前端。尽管此时有未预构建的第三方依赖，依然先用 esbuild 处理返回给前端，然后再重新预构建。返回给前端的内容，已经可以供页面展示了，此时尽管预构建的内容已经发生了变化，但是不影响页面使用。当发生懒加载，重新预构建时，预构建的内容又发生了变化，这个时候就需要重新加载页面了(why？？)。
