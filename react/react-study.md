...
- [x] `useCallback` 滥用问题
  ...
  很多人喜欢在传递给子组件的 `callback` 用 `useCallback` 处理，认为这样就可以实现子组件不重复渲染。

  实际上，每次父组件渲染的时候，传递给子组件的 `props` 都是一个新的对象，只是这个 `props` 里面的属性值可能和上一个 `props` 相等，子组件依旧会触发重复渲染。

  这个时候，需要配合 `react.memo` 使用。

  滥用 `useCallback`，会造成不必要的缓存，影响性能, 还会造成复杂性。
