...
10. 依赖预构建过程是怎么样的?
    ...
    3. 开始依赖预构建
        ...
        依赖预构建比较关键的一步，就是找到项目依赖的第三方库，如 react、react-dom 等。
...
17. 第一次启动本地服务的时候，会先去判断需不需要进行预构建，然后启动本地服务。如果不需要预构建，直接使用上一次预构建的数据；如果需要，那么会在本地服务启动以后，立刻进行预构建。

18. 项目中的业务代码是否支持 commonjs 写法 ？

    目前看是不支持的。预构建的时候也不会对 cjs 模块进行处理。

19. vite 本地服务启动以后使用到的几个中间件

   

20. vite 中 index.html、 js、 css 文件是怎么处理的？
