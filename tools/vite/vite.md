...
11. 预构建过程中的 hash 和 browserHash 是什么意思？
...
15. 每次启动时，如何判断需要是否需要预构建？上一次预构建的内容是否可以使用？
...
16. 第一次启动本地服务的时候，会先去判断需不需要进行预构建，然后启动本地服务。如果不需要预构建，直接使用上一次预构建的数据；如果需要，那么会在本地服务启动以后，立刻进行预构建。

17. 项目中的业务代码是否支持 commonjs 写法 ？

    目前看是不支持的。预构建的时候也不会对 cjs 模块进行处理。

18. vite 本地服务启动以后使用到的几个中间件

   

19. vite 中 index.html、 js、 css 文件是怎么处理的？
